...
2. **Yocto (a custom `meta-hydra-umc` layer)** — full control, minimal
   ...
   Yocto BSP layer for CM5 + Hailo-8 + PREEMPT_RT is itself a project, not
   a weekend), steeper for anyone else contributing to reproduce.
...
- A real, tested boot chain: bootloader config, kernel/device-tree for CM5
  + this project's own custom hardware (Hailo-8 M.2, dual GL3523, STM32H745
  SPI link) — none of the device-tree overlays this custom board will need
  exist yet
