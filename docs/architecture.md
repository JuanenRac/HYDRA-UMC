...
| Robot Controller Board <-> URTC Tool Head relay tunnel | **IMPLEMENTED** - `src/mcu_stm32g474/RobotControllerRelay.c` is the real application-side relay logic this row used to flag as missing: FDCAN2 (downlink) init, a real AXIS_STATUS (+0x10) responder reading this board's own endstop/fault GPIOs, and a real RELAY_SEND/RELAY_RECV tunnel forwarding opaque frames to/from the URTC head - the exact real fragmentation scheme `KinematicBrainCan.c` (Tier 0) and `spi_bridge/relay_tunnel.py` (CM5) both already implement. Compiles and links clean (`build_firmware.sh g474`). `OFS_ENTER_BOOTLOADER` (+0x00) is intentionally still not implemented - see that module's own header for why. NOT yet verified against real hardware. |
