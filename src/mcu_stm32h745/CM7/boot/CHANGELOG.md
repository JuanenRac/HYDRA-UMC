...
| **0.0.0** | Initial versioned release. Real CAN-OTA bootloader, mailbox-relayed (this core has no bus of its own - see this component's own `bootloader_protocol.c`), implementing CRC32 + HMAC-SHA256 verify-into-backup-before-copy-to-main - compiling clean end to end, not yet verified against real hardware. |
