# Changelog - Robot Controller Board Application (`src/mcu_stm32g474/`)
...
| **0.0.6** | Found during the same audit that fixed `boot/bootloader_common.h`'s own IWDG init ordering (hallazgo #110): this application inherits the watchdog already armed by its own bootloader (an IWDG, once started, survives `HAL_DeInit()`) but never touched it after handoff - on real hardware this would have reset in a perpetual loop the moment the bootloader's own IWDG timeout elapsed. Added this application's own `IWDG_HandleTypeDef` (same Instance/Prescaler/Reload as the bootloader) plus a periodic refresh in `vBlinkTask`. (0.0.4 was a verification build, no source changes; 0.0.5 was an orphaned intermediate build superseded by this one.) |
...
| **0.0.0** | Initial versioned release (previously `MAJOR.MINOR` only, no PATCH field). Still a FreeRTOS GPIO-toggle smoke test, not yet the real motion/relay application logic - see `README.md` for current status. |
