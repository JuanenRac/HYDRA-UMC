# Changelog - Robot Controller Board Bootloader (`src/mcu_stm32g474/boot/`)
...
1, e.g. `0.1.9` -> `0.2.0`, never `0.1.10`; MINOR past 9 carries into MAJOR
...
| **0.0.6** | Fixed a real defect found in a full-ecosystem audit (hallazgo #110): `HAL_IWDG_Init()` was being called *after* `HAL_Init()`/`SystemClock_Config()` in `main()`, leaving the startup window before that point completely unwatched by the watchdog - moved to the first line of `main()`, before `HAL_Init()`, so a hang anywhere during clock/peripheral bring-up is actually caught. (0.0.4 was a verification build, no source changes; 0.0.5 was an orphaned intermediate build superseded by this one.) |
...
| **0.0.0** | Initial versioned release. Real CAN-OTA bootloader (see this component's own `bootloader_protocol.c`) implementing CRC32 + HMAC-SHA256 verify-into-backup-before-copy-to-main, same anti-bricking discipline as URTC's own bootloader - compiling clean end to end, not yet verified against real hardware. |
