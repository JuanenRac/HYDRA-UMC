...
//
// Ported verbatim from URTC's own proven bootloader_crypto.c (same repo's
// own RFC 4231 test-case-2 verification applies unchanged - this is the
// exact same algorithm, no chip-specific instructions anywhere in it).
// Only HAL_IWDG_Refresh(&hiwdg) ties this to a specific chip family, and
// that call has the same signature on G4 as it did on F3.
// =============================================================================
...
// PLACEHOLDER default - change before relying on this for anything real,
// and keep whatever build/signing tool prepares HYDRA-UMC update images in
// sync. Deliberately different from URTC's own HMAC_KEY (bootloader_crypto.c
// in that repo) - two separate trust domains, a URTC-signed image must
// never verify against a HYDRA-UMC board's key and vice versa.
const uint8_t HMAC_KEY[32] = {
    0x48, 0x59, 0x44, 0x52, 0x41, 0x2D, 0x55, 0x4D,
    0x43, 0x2D, 0x52, 0x43, 0x42, 0x2D, 0x32, 0x30,
    0x32, 0x36, 0x2D, 0x43, 0x48, 0x41, 0x4E, 0x47,
    0x45, 0x2D, 0x4D, 0x45, 0x2D, 0x21, 0x21, 0x21
};
...
static void sha256_update(SHA256_CTX *ctx, const uint8_t data[], uint32_t len) {
    for (uint32_t i=0; i<len; ++i) {
        ...
        // A single call here can span the full 236KB backup slot (via
        // hmac_sha256_flash_region) - every 4096 bytes keeps the IWDG
        // refresh frequent without adding meaningful overhead to the hot
        // loop, same reasoning/interval as URTC's own bootloader.
        if ((i & 0xFFF) == 0) {
